...
RSpec.describe UsersController, type: :request do
  ...
  describe '#show' do
    ...
    context 'a waiting user has 4 mature pull_requests' do
      ...
      it 'transitions the user to the completed state', vcr: { record: :new_episodes } do
        get profile_path
        user.reload
    ...
    context 'a user has more than 4 eligible pull requests' do
      ...
      it 'transitions the user to the waiting state', vcr: { record: :new_episodes } do
        allow_any_instance_of(User)
          .to receive(:eligible_pull_requests_count).and_return(4)
    ...
    context 'a user has no pull_requests' do
      ...
      it 'keeps the user in the registered state', vcr: { record: :new_episodes } do
        allow_any_instance_of(User)
